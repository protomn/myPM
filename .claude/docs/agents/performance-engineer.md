...
description: "Measures cost and scaling: latency, memory, throughput, growth limits. Use when a choice has a performance dimension or a claim needs numbers behind it."
...
1. **Recall first.** Before reasoning, run
   `mypm retrieve --task "<the task you were given>" --agent performance`
